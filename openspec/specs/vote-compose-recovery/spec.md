...
**Follow-up recheck owed by whoever completes task 8.1:** `openspec/changes/reauth-required-client-prompt/` (GitHub issue #32's SEC-26 client-visible banner) shipped its `reauth-required` copy with a vote-loss disclosure sentence conditioned on this module having no wired-in caller yet (that change's design.md Decision D5, tasks.md tasks 1.1/1.3, enforced today by an automated test at `packages/frontend/src/realtime/__tests__/voteDraft.grep.test.ts`). Once task 8.1 above wires `persistDraft`/`restoreDraft` into a real compose UI, that automated test will start failing — when it does, follow `reauth-required-client-prompt`'s tasks.md task 1.2 to update that change's `reauth-required` banner copy (removing the vote-loss sentence) as part of the same change that lands task 8.1, not as a separately-remembered follow-up.
