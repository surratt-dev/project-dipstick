...
Defines the local-development-only persona login shortcut: an endpoint that surfaces available seeded IdP test accounts, a `login_hint` passthrough on the sign-in initiation route, and a landing page that lets a developer pick a persona instead of typing an account id into the simulated IdP's form. The full Authorization Code + PKCE exchange still runs for every sign-in; this capability only affects which account the IdP's own interaction screen pre-selects. Double-gated to be unreachable in any production deployment.
