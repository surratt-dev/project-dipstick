...
- **WHEN** the authorization helper is called for a facilitator whose only session row for the requested team has `status = 'completed'` and `facilitator_access_expires_at < NOW()`
...
- **WHEN** the authorization helper is called for a facilitator whose session for the requested team has `status = 'completed'` and `facilitator_access_expires_at > NOW()`
- **THEN** the helper returns a grant with `path = 'facilitator'` and `sessionStatus = 'completed'`
...
Application Admins (users with `global_role = 'application_admin'`) SHALL have access to team administrative data endpoints only. They SHALL NOT have access to session content endpoints.
...
- A `draft` session that is not advanced to `lobby` within 24 hours of creation is automatically deleted. The automatic deletion does not transition through any other status — it is a hard delete of the session record.
- The facilitator's access grant under a `draft` session expires when the `draft` session is deleted (either advanced or auto-expired).

**Authorization SQL for draft sessions:** `draft` is included in the `sessions.status IN (...)` clause of the facilitator access check (see Requirement: Authorization helper evaluates three access paths).
...
When a session transitions to `completed`, the application SHALL set `sessions.facilitator_access_expires_at = completed_at + INTERVAL '30 minutes'`. During this window, the facilitator retains read-only access to the team's historical session data, trend data, and action items. After the window expires, the facilitator's Path 3 check fails and access ends.
...
- `facilitator_access_expires_at` is set server-side at session completion. It MUST NOT be updatable by the facilitator or any client call.
- The grace window applies to the most recently completed session row. It does not accumulate across sessions: a facilitator who ran two sessions for Team A has at most one active grace window at any time (for the most recently completed session).
...
- **WHEN** a session for Team A transitions to `completed` at time T
