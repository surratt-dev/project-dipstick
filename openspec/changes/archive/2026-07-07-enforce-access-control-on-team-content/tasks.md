...
- [ ] 11.10 End-to-end test exercising all four facilitator live session error states through the full stack (HTTP through WebSocket through UI response) **Backend/WebSocket half now has real full-stack coverage, delivered by `facilitator-error-states-e2e`:** Error States 1 (both the recoverable and non-recoverable reveal-failure paths), 1a, 1b (backend contract only), 2, 3 (real-trigger case), and 4 are proven against real Postgres and real Redis — no mocked `db.query` anywhere in the covering tests — via `packages/backend/src/routes/__tests__/facilitator-error-states-integration.test.ts` and `facilitator-error-state-2-restricted-role.test.ts`, run through the existing `integration` CI workflow (`.github/workflows/integration.yml`) with no new CI wiring required. Error State 3's coverage is asserted against its real production trigger — a real committed session-state transition (via the topic-advance wrap-up-entry branch or session completion) that publishes `session_state_change` and delivers it to a real subscriber over the actual Redis pub/sub channel, not a directly-invoked function call or a manually-published stand-in event. Error State 3's "system timeout" trigger variant remains explicitly out of scope: no timeout-driven auto-transition mechanism exists anywhere in this codebase, so there is nothing to trigger — this is a separate, unbuilt mechanism, not a consequence of issue #26 (issue #26 is fully resolved). **Still BLOCKED, and the box remains unchecked**, because no frontend WebSocket client or UI rendering of these six states exists yet in `packages/frontend` — that is tracked as [issue #38](https://github.com/surratt-dev/project-dipstick/issues/38), the only change that may check this box. Re-run this task once issue #38 lands.
