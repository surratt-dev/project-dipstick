...
- **AND** the response's `status` field is `'active'`, with a populated `currentTopic` for the new topic (design.md Decision D4a)
...
- **AND** the response's `status` field is `'wrap_up'`, with `wrapUpStartedAt` set and no `currentTopic` (design.md Decision D4a)
- **AND** a `session_state_change` WebSocket event is published to the session's subscribers after the transaction commits
