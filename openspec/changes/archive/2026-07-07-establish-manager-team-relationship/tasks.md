...
- [x] 1.6 Conduct threat modeling session covering the four scenarios named in design.md Phase 0: TEAM-006 as an access escalation vector, EM account compromise, IdP claim manipulation, and retroactive historical data exposure under Decision 6; session must complete and findings must be documented before Phase 2 begins; the threat model must also name the action item inference risk (Decision 13) as a known inference path, not a boundary violation — **COMPLETE: threat model conducted and documented in `threat-model.md`, reviewed by the solution architect and internal champion (see `threat-model-review-architect.md` and `threat-model-review-champion.md`), all four required scenarios and the Decision 13 inference risk covered. Session found a real High-severity gap (finding 1.3: `evaluateTeamAccess` grants EM data access from `team_memberships.role` alone, bypassable via TEAM-005 without ever calling TEAM-006) plus two further gaps (TEAM-006 rate limiting per task 3.10 not yet built; EM account monitoring named in Decision 6 unverified). Per the threat model's own recommendation, finding 1.3 and task 3.10 must be resolved, and the EM-monitoring disposition confirmed, before Phase 2 ships to production — this task (conducting and documenting the threat model) is complete; Phase 2 production readiness is not**
