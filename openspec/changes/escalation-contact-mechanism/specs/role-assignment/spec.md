...
- **WHEN** a user without role-assignment permission views the member management view
- **THEN** the escalation message shows the Application Admin contact mechanism (name + `mailto:` link for each current Application Admin)
- **AND** no attempt is made to reference Engineering Manager data, because none exists to reference

#### Scenario: TEAM-005 escalation shows the zero-admin fallback when no real Application Admin exists
